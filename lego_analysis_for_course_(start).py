...
Original file is located at
    https://colab.research.google.com/drive/171hQq6fN1x2BTZ1lSNApuALzQeoOHrmY
...
# Displaying the number of sets released year-on-year
...
t_year = sets.groupby('year').agg({'theme_id': pd.Series.nunique})

t_year.rename(columns={'theme_id':'nr_themes'},inplace =True)
...
plt.plot(t_year.index[:-2],t_year.nr_themes[:-2])
...
average_parts_per_set = sets.groupby('year')['num_parts'].agg('mean')

average_parts_per_set.head()

average_parts_per_set.tail()
...
plt.scatter(average_parts_per_set.index[:-2], average_parts_per_set.values[:-2])
plt.xlabel('Release Year')
plt.ylabel('Average Number of Parts per Set')
plt.title('Average Number of Parts per Set by Release Year')
plt.show()
...
df=pd.read_csv("/content/data/themes.csv")
